...
class LayerNorm(nn.Module):
    ...
    def forward(
        ...
        return (
            (residual - mean) / t.sqrt(var + self.cfg.layer_norm_eps)
        ) * self.w + self.b
...
class PosEmbed(nn.Module):
    ...
    def forward(
        ...
        positional_values = self.W_pos[range(tokens.shape[1])].repeat(
            tokens.shape[0], 1, 1
        )
...
class Attention(nn.Module):
    ...
    def apply_causal_mask(
        ...
        """
        mask = t.triu(t.ones_like(attn_scores), diagonal=1) == 0
        return attn_scores.where(mask, self.IGNORE)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        K = (
            einops.einsum(normalized_resid_pre, self.W_K, "b s e, n e h -> b s n h")
            + self.b_K
        )
        Q = (
            einops.einsum(normalized_resid_pre, self.W_Q, "b s e, n e h -> b s n h")
            + self.b_Q
        )
        V = (
            einops.einsum(normalized_resid_pre, self.W_V, "b s e, n e h -> b s n h")
            + self.b_V
        )
        A = einops.einsum(Q, K, " b sq n h ,b sk n h-> b n sq sk")
        A = self.apply_causal_mask(A) / math.sqrt(self.cfg.d_head)
        A = A.softmax(-1)
        z = einops.einsum(V, A, "b sk n h, b n sq sk -> b sq n h")
        result = einops.einsum(z, self.W_O, "b sq n h, n h e -> b sq e") + self.b_O
        return result
...
class MLP(nn.Module):
    ...
    def forward(
        ...
        l1 = (
            einops.einsum(normalized_resid_mid, self.W_in, "b s e, e d -> b s d")
            + self.b_in
        )
        l_act = gelu_new(l1)
        l2 = einops.einsum(l_act, self.W_out, "b s d, d e -> b s e") + self.b_out
        return l2
...
class TransformerBlock(nn.Module):
    ...
    def forward(
        ...
        residual = self.attn(self.ln1(resid_pre)) + resid_pre
...
class Unembed(nn.Module):
    ...
    def forward(
        ...
        return (
            einops.einsum(normalized_resid_final, self.W_U, "b s e, e v -> b s v")
            + self.b_U
        )
...
class TransformerTrainingArgs:
    ...
    wandb_project: str | None = "arena-day1-demotransformer"
...
class TransformerTrainer:
    def __init__(self, args: TransformerTrainingArgs, model: DemoTransformer):
        ...
        self.model = model
        self.args = args
        self.sampler = solutions.TransformerSampler(
            self.model, reference_gpt2.tokenizer
        )
    ...
    def training_step(
        ...
        log_probs = get_log_probs(logits, tokens).mean()
        loss = -log_probs
        print(loss.shape)
        loss.backward()
        self.optimizer.step()
...
class TransformerSampler:
    ...
    def sample(
        ...
        for _ in range(max_tokens_generated):
            ...
            logits = self.model(tokens.repeat(1, 1))[0, -1, :]
            next_token = self.sample_next_token(tokens, logits, **kwargs)
            prompt += self.tokenizer.decode(next_token)
    ...
    def apply_frequency_penalty(
        ...
        return (
            logits
            - t.bincount(input_ids, minlength=logits.shape[0]).to(logits.device)
            * freq_penalty
        )
    ...
    def sample_top_p(
        ...
        """
        sorted_logits, sorted_logit_ids = logits.sort(descending=True)
        sorted_probs = sorted_logits.softmax(-1)
        cumulative_probabilities = sorted_probs.cumsum(-1)
        to_keep = max(
            min_tokens_to_keep,
            t.searchsorted(cumulative_probabilities, top_p, side="left").item() + 1,
        )
        keep_ids = sorted_logit_ids[:to_keep]
        keep_logits = logits[keep_ids]
        sampled_token_idx = t.distributions.categorical.Categorical(
            logits=keep_logits
        ).sample()
        return keep_ids[sampled_token_idx].item()
