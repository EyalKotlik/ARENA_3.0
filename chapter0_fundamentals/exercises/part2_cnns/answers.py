...
def train(args: SimpleMLPTrainingArgs) -> tuple[list[float], list[float], SimpleMLP]:
    ...
    mnist_testloader = DataLoader(
        mnist_testset, batch_size=args.batch_size, shuffle=True
    )
...
class Conv2d(nn.Module):
    def __init__(
        ...
        sqrt = np.sqrt(self.in_channels * self.kernel_size * self.kernel_size)
        self.weight = nn.Parameter(
            2
            * t.rand(
                [
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.training:
            ...
            variance = t.var(x, dim=[0, 2, 3], unbiased=False)
            self.running_mean = (
                self.momentum * mean + (1 - self.momentum) * self.running_mean
            )
            self.running_var = (
                self.momentum * mean + (1 - self.momentum) * self.running_var
            )
        ...
        return ((x - reshape(mean)) / t.sqrt(reshape(variance) + self.eps)) * reshape(
            self.weight
        ) + reshape(self.bias)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_feats: int, out_feats: int, first_stride=1):
        ...
        if not is_shape_preserving:
            ...
            self.right_branch = Sequential(nn.Identity())
...
class ResNet34(nn.Module):
    def __init__(
        ...
        n_groups = len(self.n_blocks_per_group)
        ...
        self.net = Sequential(
            ...
            Sequential(
                BlockGroup(
                    self.n_blocks_per_group[0],
                    out_feats0,
                    self.out_features_per_group[0],
                    self.first_strides_per_group[0],
                ),
                *[
                    BlockGroup(
                        self.n_blocks_per_group[i],
                        self.out_features_per_group[i - 1],
            ...
            AveragePool(),
            Linear(self.out_features_per_group[-1], n_classes),
        )
...
pretrained_resnet = models.resnet34(weights=models.ResNet34_Weights.IMAGENET1K_V1).to(
    device
)
...
def predict(
    ...
    model.eval()
    result = model(images)
    predicted_class = t.softmax(result, dim=-1)
    return predicted_class.max(dim=-1)
...
for i, img in enumerate(images):
    ...
    table.add_row("My ResNet", imagenet_labels[my_predictions[i]], f"{my_probs[i]}")
    table.add_row(
        "Reference Model",
        imagenet_labels[pretrained_predictions[i]],
        f"{pretrained_probs[i]}",
    )
    rprint(table)
    display(img)
...
def get_resnet_for_feature_extraction(n_classes: int) -> ResNet34:
    ...
    resnet.net[-1] = Linear(resnet.out_features_per_group[-1], n_classes)
    return resnet
...
def train(args: ResNetTrainingArgs) -> tuple[list[float], list[float], ResNet34]:
    ...
    cifar_testloader = DataLoader(
        cifar_testset, batch_size=args.batch_size, shuffle=True
    )
