...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    broadcasted_dims = len(broadcasted.shape)
    original_dims = len(original.shape)
    added_dims = broadcasted_dims - original_dims

    for _ in range(added_dims):
        broadcasted = np.sum(broadcasted, axis=0, keepdims=False)
    broadcasted_dims = len(broadcasted.shape)
    assert (
        broadcasted_dims == original_dims
    ), "broadcasted must have added dimensions only to the left"

    for i in range(original_dims):
        if broadcasted.shape[i] != original.shape[i]:
            assert original.shape[i] == 1, "original dim must be 1"
...
def log_forward(x: Tensor) -> Tensor:
    ...
    out = Tensor(
        np.log(x.array), requires_grad=grad_tracking_enabled and x.requires_grad
    )
...
def multiply_forward(a: Tensor | int, b: Tensor | int) -> Tensor:
    ...
    if requires_grad:
        out.recipe = Recipe(
            np.multiply, args=(arg_a, arg_b), kwargs={}, parents=parents
        )
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        ...
        arg_tensors_check = tuple([isinstance(x, Tensor) for x in args])
        requires_grad = (
            is_differentiable
            and grad_tracking_enabled
            and any(
                arg_tensors_check[i] and x.requires_grad for (i, x) in enumerate(args)
            )
        )
        out = Tensor(
            array=numpy_func(*arg_arrays, **kwargs), requires_grad=requires_grad
        )
        if requires_grad:
            parents = {}
            for i, x in enumerate(args):
                if arg_tensors_check[i]:
                    parents[i] = x
            out.recipe = Recipe(
                numpy_func, args=arg_arrays, kwargs=kwargs, parents=parents
            )
    ...
    for node in graph:
        if not node.is_leaf:
            ...
            for i, parent in recipe.parents.items():
                if parent.requires_grad == False:
                    continue
                back_fn = BACK_FUNCS.get_back_func(recipe.func, i)
                grads[parent] = grads.get(parent, 0) + back_fn(
                    grads[node], node.array, *recipe.args, **recipe.kwargs
                )
        if node.is_leaf and node.requires_grad == True:
...
def sum_back(grad_out: Arr, out: Arr, x: Arr, dim=None, keepdim=False):
    ...
    result = np.expand_dims(grad_out, dim) if dim is not None else grad_out
    return (
        np.broadcast_to(result, x.shape)
        if keepdim == False
        else np.broadcast_to(grad_out, x.shape)
    )
...
BACK_FUNCS.add_back_func(
    np.subtract, 1, lambda grad_out, out, x, y: -1 * unbroadcast(grad_out, y)
)
BACK_FUNCS.add_back_func(
...
BACK_FUNCS.add_back_func(
    np.true_divide,
    1,
    lambda grad_out, out, x, y: unbroadcast(-x * grad_out / (y**2), y),
)
...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        sf = np.sqrt(in_features)
        self.weight = Parameter(
            Tensor(2 * np.random.rand(out_features, in_features) / sf - 1 / sf)
        )
        self.bias = Parameter(Tensor(2 * np.random.rand(out_features) / sf - 1 / sf))
...
def cross_entropy(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    logits = logits - np.max(logits.array,axis=1,keepdims=True)
