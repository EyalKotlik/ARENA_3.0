...
class SGD:
    ...
    def step(self) -> None:
        ...
        for i in range(len(self.params)):
            ...
            if self.mu != 0:
                self.b[i].copy_(self.mu * self.b[i] + grad)
                grad = self.b[i]
...
class RMSprop:
    ...
    def step(self) -> None:
        for i in range(len(self.params)):
            ...
            if self.lmda != 0:
                grad = grad + self.lmda * self.params[i]
            self.v[i] = self.alpha * self.v[i] + (1 - self.alpha) * (grad.square())
...
class Adam:
    ...
    def step(self) -> None:
        for theta, m, v in zip(self.params, self.m, self.v):
            g = theta.grad if theta.grad is not None else t.zeros_like(theta)
            if self.lmda != 0:
                g = g + self.lmda * theta
            m.copy_(self.beta1 * m + (1 - self.beta1) * g)
            v.copy_(self.beta2 * v + (1 - self.beta2) * g.square())
...
class WandbResNetFinetuningArgs(ResNetFinetuningArgs):
    ...
    wandb_project: str | None = "arena-day3-resnet"
...
sweep_config = dict(
    ...
    parameters=dict(
        ...
        weight_decay_active=dict(values=[0, 1]),
...
def update_args(
    ...
    new_args = WandbResNetFinetuningArgs(
        args.n_classes,
        sampled_parameters["batch_size"],
        args.epochs,
        sampled_parameters["lr"],
        (
            sampled_parameters["weight_decay"]
            if sampled_parameters["weight_decay_active"] == 1
            else 0
        ),
        args.wandb_project,
        args.wandb_name,
