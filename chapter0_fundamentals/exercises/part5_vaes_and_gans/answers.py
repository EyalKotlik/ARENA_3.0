...
class AutoencoderArgs:
    ...
    # logging
    use_wandb: bool = True
    wandb_project: str | None = "arena-day5-autoencoder"
...
class AutoencoderTrainer:
    ...
    def training_step(
        ...
        if enabled.
        """
        out_img = self.model(img.to(device))
        loss = nn.MSELoss()(img.to(device), out_img)
        loss.backward()
        self.optimizer.step()
...
class VAE(nn.Module):
    ...
    def __init__(self, latent_dim_size: int, hidden_dim_size: int):
        ...
        self.encoder = Sequential(
            ...
            Rearrange("b (dim ldim) -> dim b ldim", dim=2),
        )
        self.decoder = Sequential(
            Linear(latent_dim_size, hidden_dim_size),
            ReLU(),
    ...
    def sample_latent_vector(self, x: Float[Tensor, "batch 1 height width"]) -> tuple[
    ...
    def forward(self, x: Float[Tensor, "batch 1 height width"]) -> tuple[
        Float[Tensor, "batch 1 height width"],
        Float[Tensor, "batch latent"],
        Float[Tensor, "batch latent"],
    ]:
        ...
        and logsigma.
        """
        samples, mu, logsigma = self.sample_latent_vector(x)
        return self.decoder(samples), mu, logsigma
...
class VAETrainer:
    ...
    def training_step(
        ...
        if enabled.
        """
        out_img, mu, logsigma = self.model(img)
        loss = (
            nn.MSELoss()(img, out_img)
            + self.args.beta_kl
            * ((logsigma.exp() ** 2 + mu**2 - 1) / 2 - logsigma).mean()
        )
        loss.backward()
        self.optimizer.step()
        self.optimizer.zero_grad()
...
class LeakyReLU(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return x.maximum(self.negative_slope * x)
...
tests.test_initialize_weights(
    initialize_weights, nn.ConvTranspose2d, Conv2d, Linear, BatchNorm2d
)
# %%
model = DCGAN().to(device)
x = t.randn(3, 100).to(device)
...
class DCGANTrainer:
    ...
    def training_step_discriminator(
        ...
        loss = -(t.log(1 - D_fake).mean() + t.log(D_real).mean())
        loss.backward()
        self.optD.step()
        self.optD.zero_grad()
